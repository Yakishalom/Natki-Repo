...
import requests
from cocoscrapers.modules.control import setting as getSetting, setSetting
		...
		for i in files:
			try:
				if i['is_ready'] == '1' and i['type'] == 'video':
					...
					append({'provider': 'furk', 'source': source, 'name': name,'seeders':0, 'name_info': name_info, 'quality': quality, 'language': "en", 'url': url,
