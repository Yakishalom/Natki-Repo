...
class source:
	...
	def __init__(self):
		...
		self.min_seeders = 2 # to many items with no value but cached links
	...
	def sources(self, data, hostDict):
		...
		for row in rows:
			try:
				...
				info = ' | '.join(info)
				host='load.to'
				append({'provider': 'glodls', 'source': host, 'seeders': seeders,'name': name+' '+str(seeders), 'name_info': name_info,
	...
	def get_sources_packs(self, link):
		...
		for row in rows:
			try:
				...
				info = ' | '.join(info)
				host='load.to'
				item = {'provider': 'glodls', 'source': host, 'seeders': seeders, 'name': name+' '+str(seeders), 'name_info': name_info, 'quality': quality,
							'language': 'en', 'url': url, 'info': info, 'direct': 'elementum', 'debridonly': True, 'size': dsize, 'package': package}
