	...
	def sources(self, data, hostDict):
		...
		for row in rows:
			try:
				...
				info = ' | '.join(info)
				host='load.to'
				append({'provider': 'magnetdl', 'source': host, 'seeders': seeders, 'name': name+' '+str(seeders), 'name_info': name_info,
	...
	def get_sources_packs(self, url):
		...
		for row in rows:
			try:
				...
				info = ' | '.join(info)
				host='load.to'
				item = {'provider': 'magnetdl', 'source': host, 'seeders': seeders, 'name': name+' '+str(seeders), 'name_info': name_info, 'quality': quality,
