	...
	def get_sources(self, item):
		try:
			...
			if 40<int(item[5]):
				...
				self.sources_append({'provider': 'torrents', 'source': host, 'seeders': item[5],  'name': item[0]+' '+str(item[5]), 'name_info': item[1],
												'quality': quality, 'language': 'en', 'url': url, 'info': info, 'direct': 'elementum', 'debridonly': False, 'size': item[4]})
		except:
			source_utils.scraper_error('1337X')