	...
	def sources(self, data, hostDict):
		...
		for file in files.get('content'):
			try:
				...
				append({'provider': 'bitlord', 'source': host, 'seeders': seeders, 'name': name+' '+str(seeders), 'name_info': name_info,
	...
	def get_sources_packs(self, link, url):
		...
		for file in files.get('content'):
			try:
				...
				info = ' | '.join(info)
				host='load.to'
				item = {'provider': 'bitlord', 'source': host, 'seeders': seeders, 'name': name+' '+str(seeders), 'name_info': name_info, 'quality': quality,
