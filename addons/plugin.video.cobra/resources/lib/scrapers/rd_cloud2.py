...
class source:
	...
	def results(self, info):
		try:
			...
			self.media_type, title, self.tmdb_id, self.title_heb = info.get('media_type'), info.get('title'), info.get('tmdb_id'),info.get('title_heb')
			...
			def _process():
				...
				for item in self.scrape_results:
					try:
						...
						if filter_title and not item['assigned_content'] and not check_title(title, file_name, aliases, self.year, self.season, self.episode): continue
			...
			from modules.kodi_utils import logger
			logger('COBRA real-debrid scraper Exception', str(e))
		internal_results(self.scrape_provider, self.sources)
		return self.sources

	def _gather_assigned_content(self):
		pre_assigned_content = gather_assigned_content("'FEN_RD_%'")
	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				normalized = item['filename']#normalize(item['filename'])
				if self.is_hebrew(str(normalized)):
					normalized = item['filename']
				...
				if test_assigned_content('FEN_RD_%s' % item['id'], self.pre_assigned_content): results_append((item['id'], True))
				elif not folder_name: results_append((item['id'], False))
	...
	def _scrape_folders(self, folder_info):
		try:
			folder_files = RealDebrid.user_cloud_info(folder_info[0])
	...
	def _scrape_downloads(self):
		try:
			...
			for item in my_downloads:
				...
				normalized = item['filename']
				if self.is_hebrew(str(normalized)):
					normalized = str(self.year)+item['filename']
				else:
					normalized = normalize(item['filename'])
				...
				if self.is_hebrew(str(normalized)):
					...
					if not self.title_heb.replace(' ','').replace("'",'') in folder_name: continue
				else:
	...
	def make_downloads_item(self, item):
		return {'assigned_content': False, 'url_link': item['download'], 'bytes': item['filesize'], 'path': item['filename'], 'direct_debrid_link': True}
