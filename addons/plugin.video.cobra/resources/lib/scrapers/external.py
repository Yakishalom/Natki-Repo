...
class source:
	def __init__(self, meta, source_dict, debrid_torrents, debrid_hosters, internal_scrapers, prescrape_sources, progress_dialog, disabled_ext_ignored=False):
		...
		self.sleep_time = 100
		self.finish_early = get_setting('cobra.search.finish.early') == 'true'
		self.timeout = 60 if disabled_ext_ignored else int(get_setting('cobra.results.timeout', '20'))
	...
	def check_subtitle(self,tv_movie,original_title,name,season,episode,year,id): # telemedia
			from subs import main
			from caches import sub_cache
			f_subs=[]
			try:
				f_subs=sub_cache.get(main.get_links,24,tv_movie,original_title,name,season,episode,year,id, table='posters')
			except:pass
			from modules.kodi_utils import subs, database
			dbcon = database.connect(subs)
			dbcur = dbcon.cursor()
			dbcur.execute("CREATE TABLE IF NOT EXISTS %s ( ""name TEXT ,""free TEXT);" % ('name_sub'))
			dbcur.execute("SELECT * FROM name_sub ")
			name_sub = dbcur.fetchall()
			all_subs=[]
			dbcon.execute("DELETE FROM name_sub")
			dbcon.commit()
			import logging
			for i in f_subs:
					if i in all_subs:
						continue
					all_subs.append(i)
					dbcur.execute("INSERT INTO name_sub Values ('%s','%s')"%(i,' '))
					dbcon.commit()
			dbcon.close()
	def results(self, info):
		if not self.source_dict: return
		try:
			self.media_type, self.tmdb_id, self.imdb_id, self.orig_title = info['media_type'], str(info['tmdb_id']), str(info['imdb_id']), info['title']
			...
			if self.media_type == 'movie':
				...
				self.data = {'imdb': info['imdb_id'], 'title': self.title, 'aliases': aliases, 'year': self.year, 'title_heb': info['title_heb']}
			else:
				...
				self.data = {'imdb': info['imdb_id'], 'tvdb': info['tvdb_id'], 'tvshowtitle': self.title, 'aliases': aliases,'year': self.year,
							'title': ep_name, 'season': str(self.season), 'episode': str(self.episode), 'title_heb': info['title_heb']}
			try:# telemedia
				# self.check_subtitle('movie',self.title,self.title,'0','0',self.year,self.imdb_id)
				self.start_sub = Thread(target=self.check_subtitle, args=(self.media_type,self.title,self.title,self.season,self.episode,self.year,self.imdb_id,))
	...
	def get_sources(self):
		...
		def _background():
			# sleep(1500) # cobra
			sleep(1000)# telemedia
			end_time = time.time() + self.timeout
			while time.time() < end_time:
				alive_threads = [x for x in self.threads if x.is_alive()]
				len_alive_threads = len(alive_threads)
				# sleep(1000) # cobra
				sleep(5000)# telemedia #הוספתי כי לא מספיק למצוא מקורות במקור של טלמדיה לפרק הבא
	...
	def process_hosters(self, hoster_sources):
		...
		telemedia_hosters =[{'Real-Debrid': ['rapidgator.net']},{'Real-Debrid': ['load.to']},{'Real-Debrid': ['vivo.st']},{'Real-Debrid': ['rg.to']},{'Real-Debrid': ['thevideo.me']}]
		hoster_results = []
		if not self.debrid_hosters:
			self.debrid_hosters=telemedia_hosters
