...
get_property, open_settings,open_telemedia_settings,open_scrapers_settings, set_property, get_icon, dialog = kodi_utils.get_property, kodi_utils.open_settings,kodi_utils.open_telemedia_settings,kodi_utils.open_scrapers_settings, kodi_utils.set_property, kodi_utils.get_icon, kodi_utils.dialog# telemedia
...
def trakt_manager_choice(params):
	if not get_setting('cobra.trakt.user', ''): return notification(32760, 3500)
...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	try: preselect = [fl.index(i) for i in get_setting('cobra.%s' % quality_setting).split(', ')]
	except: preselect = []
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				button_action = get_setting('cobra.%s' % setting_id)
...
def set_language_filter_choice(params):
	...
	try: preselect = [fl.index(i) for i in get_setting('cobra.%s' % filter_setting).split(', ')]
	except: preselect = []
...
def easynews_use_custom_farm_choice(params={}):
	...
	new_setting = 'True' if get_setting('cobra.easynews.use_custom_farm', 'False') == 'False' else 'False'
...
def easynews_server_choice(params={}):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': easy_serv_str, 'narrow_window': 'true'}
	port_choice = select_dialog(ports, **kwargs) or get_setting('cobra.easynews.port', '443')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or get_icon('cobra')
...
def scraper_quality_color_choice(params):
	...
	current_setting = get_setting('cobra.%s' % setting)
...
def highlight_color_choice(params={}):
	current_setting = get_setting('cobra.main_highlight')
	chosen_color = color_choice({'current_setting': current_setting})
	if chosen_color:
		manage_settings_reset()
...
def mpaa_region_choice(params={}):
	...
	manage_settings_reset(True)
	delete_meta_cache(silent=True)
def bot_telemedia(params, meta=None): # telemedia
	import xbmc,xbmcaddon

	telemedia=xbmcaddon.Addon('plugin.video.telemedia')
	if telemedia.getSetting("super_bot")=='false':
		notification(33024, 3000)
		set_setting('bot_on','true')
		telemedia.setSetting("super_bot","true")
	else:
		notification(32840, 3000)
		set_setting('bot_on','false')
		telemedia.setSetting("super_bot","false")
	xbmc.executebuiltin('Container.Refresh')
def play_trailer_eng(params, meta=None): # telemedia
	import xbmc
	params_get = params.get
	title,tmdb_id,content= params_get('title', None),params_get('tmdb_id', None),params_get('content', None)
	if content == 'tvshow':
		tv_movie= 'tv'
	else:
		tv_movie= 'movie'
	url='plugin://plugin.video.telemedia/?mode=171&id={0}&url={1}&description={2}'.format(tmdb_id,tv_movie,title)
	xbmc.executebuiltin('RunPlugin(%s)'%url)
# def movie_online(params, meta=None): # telemedia
	# params_get = params.get
	# title,tmdb_id,poster,fanart = params_get('title', None),params_get('tmdb_id', None),params_get('poster', None),params_get('fanart', None)
	# import xbmc,urllib
	# url='plugin://plugin.video.telemedia/?mode=280&url={0}&name={1}&iconimage={2}&fanart={3}'.format(tmdb_id,urllib.parse.quote_plus(title),poster,fanart)
	# xbmc.executebuiltin('RunPlugin(%s)'%url)

    
def show_torrent_sources(params, meta=None):# telemedia

	import xbmc,xbmcaddon
	cocoscrapers=xbmcaddon.Addon('script.module.cocoscrapers')
	if get_setting('show_torrent')=='false':
		notification(33024, 3000)
		set_setting('show_torrent','true')
		cocoscrapers.setSetting("torrentio_tr","true")
	else:
		notification(32840, 3000)
        
		set_setting('show_torrent','false')
	xbmc.executebuiltin('Container.Refresh')
...
def options_menu_choice(params, meta=None):
	...
	if menu_type in ('movie', 'tvshow'):
		if get_setting('cobra.trakt.user', ''): listing_append((ls(32198), '', 'trakt_manager'))
	...
	listing_append(('%s %s %s' % (ls(32641), ls(32247), ls(32533)), '', 'open_scrapers_settings'))# telemedia
	if not from_extras: listing_append(('%s %s' % (ls(32641), ls(32456)), '', 'open_tools'))
	listing_append(('%s %s' % (ls(32641), ls(32247)), '', 'open_settings'))
	listing_append(('%s %s %s' % (ls(32641), ls(32247), ls(40013)), '', 'open_telemedia_settings'))# telemedia
	list_items = list(_builder())
	...
	elif choice == 'open_scrapers_settings':# telemedia
		return open_scrapers_settings('0.0')
	elif choice == 'open_settings':
		return open_settings('0.0')
	elif choice == 'open_telemedia_settings':# telemedia
		return open_telemedia_settings('0.0')
