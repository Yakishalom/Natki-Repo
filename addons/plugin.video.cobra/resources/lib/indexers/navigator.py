...
class Navigator:
	...
	def my_content(self):
		if get_setting('cobra.trakt.user', ''):
		...
		if get_setting('cobra.imdb_user', ''):
	...
	def downloads(self):
		self.add({'mode': 'download_manager', 'folder_type': 'telemedia', 'name': 'טלמדיה'}, downloads_ins % 'טלמדיה', 'tele') # telemedia
		...
		self.add({'mode': 'remove_downloads'}, 'מחק הורדות', 'clear all')# telemedia
		self.end_directory()
	...
	def accounts_manager(self):
		trakt_active = get_setting('cobra.trakt.user') != ''
	...
	def changelog_utils(self):
		fen_clogpath = tp(log_path % 'plugin.video.cobra/resources/text')
		...
		def _process():
			for info in results:
				try:
					...
					info_tag.setPlot(' ')
					listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_title, 'iconImage': folder_icon,
		...
		def _builder():
			for item in folder_info:
				for media_type in ('movie', 'tvshow'):
					...
					display_name = get_setting('cobra.%s.display_name' % setting_id)
					if action == 'browse':
		...
		def _builder():
			for i in folders:
				try:
					...
					info_tag.setMediaType('video')
					info_tag.setPlot(' ')
					listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': folder_icon}))
					if self.is_home: listitem.setProperty('cobra.external', 'true')
		...
		def _process():
			for item_position, item in enumerate(contents):
				try:
					...
					info_tag.setPlot(' ')
					listitem.addContextMenuItems(cm)
					listitem.setProperty('cobra.context_main_menu_params', menu_editor_url)
	...
	def exit_media_menu(self):
		params = get_property('cobra.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.cobra/resources/text/tips'
	...
	def add(self, url_params, list_name, iconImage='folder', contextmenu_edit=True, original_image=False):
		...
		if contextmenu_edit:
			...
			listitem.addContextMenuItems(cm)
			listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': list_name, 'iconImage': iconImage}))
			if self.is_home: listitem.setProperty('cobra.external', 'true')
