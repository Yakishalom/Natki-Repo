	...
	def _process():
		for item in episodes_data:
			try:
                ...
                # telemedia
				bot_telemedia_params = build_url({'mode': 'play_from_bot', 'content': 'episode'})
				torrent_sources_params = build_url({'mode': 'show_torrent_sources', 'content': 'episode'})
                # telemedia
				...
				set_properties({'cobra.extras_params': extras_params, 'cobra.options_params': options_params, 'cobra.unwatched_params': unwatched_params,
								'cobra.watched_params': watched_params, 'cobra.clearprog_params': clearprog_params})
...
def build_single_episode(list_type, params={}):
	...
	handle, is_external, category_name = int(sys.argv[1]), external(), episodes_str
	item_list, unwatched = [], []
	resinsert, fen_highlight = '', get_property('cobra.main_highlight')
