...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('cobra.exit_params', folderpath)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			play_trailer_eng_params = build_url({'mode': 'play_trailer_eng', 'content': 'tvshow', 'tmdb_id': tmdb_id})#telemedia
			...
			set_properties({'cobra.extras_params': extras_params, 'cobra.options_params': options_params})
