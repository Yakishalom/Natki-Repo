...
base_str, window_prop = '[B]%s:[/B]  [I]%s[/I]', 'cobra.%s_discover_params'
...
class Discover:
		...
		def _builder():
			for count, item in enumerate(data):
				try:
					...
					listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_icon}))
	...
	def help(self):
		text_file = translate_path('special://home/addons/plugin.video.cobra/resources/text/tips/135. Discover.txt')
