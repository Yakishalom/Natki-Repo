	...
	def _builder():
		for count, item in enumerate(cloud_files, 1):
			try:
				...
				info_tag.setPlot(' ')
				listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_rd_icon,
									'service': 'RD'}))
	...
	def _builder():
		for count, item in enumerate(downloads, 1):
			try:
				...
				info_tag.setMediaType('video')
				info_tag.setPlot(' ')
				listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_rd_icon,
...
def active_days_notify_only():
	...
	try:
		...
		account_status_text = '' if days_remaining > 0 else '[COLOR red]לא בתוקף[/COLOR]'
		days_remaining_text = f' (נותרו {str(days_remaining)} ימים)' if days_remaining > 0 else ''

		notification_text = f"[B]חשבון RD: {account_status_text}{days_remaining_text}[/B]"
