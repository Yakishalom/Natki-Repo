...
bot_tele= ls(40012)# telemedia
movie_online= ls(40018)# telemedia


movie_upcoming_this_month = ls(40020)# telemedia



torrent_sources= ls(40015)# telemedia
...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('cobra.exit_params', folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
            ...
            # telemedia
			bot_telemedia_params = build_url({'mode': 'play_from_bot', 'content': 'movie', 'tmdb_id': tmdb_id})
			movie_online_params = build_url({'mode': 'movie_online', 'content': 'movie', 'title': title, 'tmdb_id': tmdb_id, 'poster': poster, 'fanart': fanart})# telemedia
			movie_upcoming_this_month_params = build_url({'mode': 'movies_dates_upcoming_this_month'})# telemedia
			play_trailer_eng_params = build_url({'mode': 'play_trailer_eng', 'content': 'movie', 'tmdb_id': tmdb_id})#telemedia

			torrent_sources_params = build_url({'mode': 'show_torrent_sources', 'content': 'movie', 'tmdb_id': tmdb_id})
            # telemedia
			...
			if meta_get('trailer')=='':#telemedia
				cm_append(('[B]קדימון באנגלית[/B]', run_plugin % play_trailer_eng_params))
			...
			if get_setting('bot_on')=='true':
				cm_append(('ניגון דרך הבוט: פעיל', run_plugin % bot_telemedia_params))
			else:
				cm_append(('ניגון דרך הבוט: כבוי', run_plugin % bot_telemedia_params))
			if get_setting('show_torrent')=='true':
				cm_append(('מקורות טורנטים: פעיל', run_plugin % torrent_sources_params))
			else:
				cm_append(('מקורות טורנטים: כבוי', run_plugin % torrent_sources_params))
                # telemedia
			cm_append((f"[B][COLOR yellow]{movie_online}[/COLOR][/B]", run_plugin % movie_online_params))# telemedia
			cm_append((f"[B][COLOR yellow]{movie_upcoming_this_month}[/COLOR][/B]", run_plugin % movie_upcoming_this_month_params))# telemedia
			


			if self.is_external:
				cm_append((refr_widg_str, run_plugin % build_url({'mode': 'kodi_refresh'})))
				set_properties({'cobra.external': 'true'})
			...
			set_properties({'cobra.extras_params': extras_params, 'cobra.clearprog_params': clearprog_params, 'cobra.options_params': options_params,
							'cobra.unwatched_params': watched_status_params, 'cobra.watched_params': watched_status_params})
	...
	def build_movie_sets_content(self, _position, _id):
		try:
			...
			if self.is_external:
				cm_append((refr_widg_str, run_plugin % build_url({'mode': 'kodi_refresh'})))
				set_property('cobra.external', 'true')
			info_tag = listitem.getVideoInfoTag()
			info_tag.setMediaType('movie')
			...
			set_property('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': title, 'iconImage': poster}))
