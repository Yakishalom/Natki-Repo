	...
	def _builder():
		for count, item in enumerate(cloud_dict, 1):
			try:
				...
				info_tag.setPlot(' ')
				listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': clean_folder_name, 'iconImage': default_ad_icon,
									'service': 'AD'}))
	...
	def _builder():
		for count, item in enumerate(links, 1):
			try:
				...
				info_tag.setPlot(' ')
				info_tag.setMediaType('video')
				listitem.setProperty('cobra.context_main_menu_params', build_url({'mode': 'menu_editor.edit_menu_external', 'name': name, 'iconImage': default_ad_icon,
