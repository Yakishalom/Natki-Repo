...
database, delete_file, navigator_db, watched_db, favorites_db,subs = kodi_utils.database, kodi_utils.delete_file, kodi_utils.navigator_db, kodi_utils.watched_db, kodi_utils.favorites_db,kodi_utils.subs# telemedia
...
def check_databases():
	try:
		from caches.providers_cache import ExternalProvidersCache # telemedia
		ExternalProvidersCache().delete_cache(silent=True)#מנקה קאש מקורות בהפעלה
	except:pass
	# try:
		# from caches.trakt_cache import clear_all_trakt_cache_data # telemedia
		# clear_all_trakt_cache_data(silent=True)#מנקה קאש טראקט בהפעלה
	# except:pass
...
def clear_all_cache():
	# if not confirm_dialog(): return # telemedia
	...
	sleep(100)
	notification(32576)# telemedia
	# ok_dialog(text=32576) # cobra original
