...
def get_title(meta):
    ...
    if custom_title: title = custom_title
        ...
        if get_setting('cobra.meta_language') == 'en': title = meta['title']
        else:
...
class Downloader:
    ...
    def run(self):
        self.download_prep()
        import xbmc,os,shutil,xbmcvfs
        if not self.action == 'meta.pack': show_busy_dialog()
        self.get_url_and_headers()
        if self.get_url_and_headers()=='telemedia':# telemedia
                # cobra_folder=os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.cobra","Telemedia Downloads/")
                cobra_folder=download_directory('telemedia')
                try:
                    if not xbmcvfs.exists(cobra_folder):
                         os.makedirs(cobra_folder)
                except:pass
                # print(f'File Size in Bytes is {file_stats.st_size}')
                # print(f'File Size in MegaBytes is {file_stats.st_size / (1024 * 1024)}')
                try:
                    source_folder = os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.telemedia","files/","documents/")
                    destination_folder = cobra_folder
                    # fetch all files
                    for file_name in os.listdir(source_folder):
                        ...
                        if not file_stats.st_size / (1024 * 1024)==0.0:
                        # move only files
                            if os.path.isfile(source):
                                shutil.move(source, destination)
                except:pass
                try:
                    source_folder = os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.telemedia","files/","videos/")
                    destination_folder = cobra_folder
                    # fetch all files
                    for file_name in os.listdir(source_folder):
                        # construct full file path
                ...
                return self.return_notification(_ok_dialog='הקובץ ירד בהצלחה ומחכה לכם באריח סרטים - הורדות')
    ...
    def download_runner(self):
        ...
        self.start_download()
    def break_download(self,id,time,requests,listen_port,random):# telemedia
    ...
    def get_url_and_headers(self):
        ...
        if url in (None, 'None', ''):
            if self.action == 'meta.single':
                ...
                try:
                    if 'telemedia' == source['provider']:# telemedia
                        l_data=json.loads(source['url'])
                        import requests,random,xbmcaddon,xbmcgui,time,sys
                        ...
                        data={'type':'td_send',
                        'info':json.dumps({'@type': 'getRemoteFile','remote_file_id':l_data['id'], '@extra':num})}
                        event=requests.post('http://127.0.0.1:%s/'%listen_port,json=data).json()

                        id=event['id']
                        data={'type':'td_send',
                        'info':json.dumps({'@type': 'downloadFile','file_id':str(id), 'priority':1,'offset':0,'limit':0,'@extra': num})}
                        event=requests.post('http://127.0.0.1:%s/'%listen_port,json=data).json()
                        start = time.time()
                        # buffer_size=8*1000000
                        dp = xbmcgui.DialogProgress()
                        dp.create('Telemedia Downloader', 'מוריד קובץ')
                        ...
                        if 'size' in event:
                            ...
                            while True:
                                ...
                                if 'path' in event:
                                    ...
                                    if dp.iscanceled():
                                        dp.close()
                                        t = Thread(target= self.break_download, args=(id,time,requests,listen_port,random,)).start()
                                        if t==None:
                                            stop=True
                                        break
                                    if file_size!=0:
                                            ...
                                            t_remiain=(buffer_size-file_size)/speed

                                            play_status='טוען באפר: '+str(int(((file_size* 100.0)/buffer_size)))+'%'
                                            dp.update(int((file_size*100.0)/buffer_size),'[B][COLOR=green]%s[/COLOR][/B]'%saved_name+'\n'+ '[B][COLOR=yellow]%s %s/%s[/COLOR][/B]'%('מבצע הורדה',str(file_size),str(buffer_size))+'\n'+str(int(speed/(1024)))+' Kbps / '+str(int(t_remiain))+' sec')
                                            if len(path)>0 and int(file_size)>=buffer_size:
                                                
                                                break
                        if stop==True:
                            self.return_notification(_ok_dialog='בוטל')
                            sys.exit()
    ...
    def _process():
        for info in results:
            try:
                ...
                # listitem.setLabel(clean_file_name(normalize(path))) # original_cobra
                listitem.setLabel(path.replace('.mp4','').replace('.mkv','').replace('.avi','').replace('.MP4','').replace('.MKV','').replace('.AVI','').replace('_',' ')) # telemedia
                # listitem.setArt({'fanart': addon_fanart,'icon': addon_fanart}) # original_cobra
                info_tag = listitem.getVideoInfoTag()
                info_tag.setMpaa(str('heb'))
                info_tag.setMediaType('files')
                info_tag.setPlot(' ')
                info_tag.setTagLine(path)
