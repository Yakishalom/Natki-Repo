# -*- coding: utf-8 -*-
from modules.kodi_utils import external, parse_qsl, get_setting,notification# telemedia import notification
# from modules.kodi_utils import logger

def sys_exit_check():
	if get_setting('cobra.reuse_language_invoker') == 'true' and external(): return True
	return False
...
def routing(sys):
	...
	if 'play_from_bot' in mode:# telemedia
			from indexers import dialogs
			return dialogs.bot_telemedia(params)
	if 'play_trailer_eng' in mode:# telemedia
			from indexers import dialogs
			return dialogs.play_trailer_eng(params)
	if 'movie_online' in mode:# telemedia
			# from indexers import dialogs
			# return dialogs.movie_online(params)
		title, original_title = _get('title'), _get('original_title')
		from modules import moviesdates
		return moviesdates.moviesdates_parser(search_movie_bool=True, title=title, original_title=original_title)
	if 'movies_dates_upcoming_this_month' in mode:# telemedia
		from modules import moviesdates
		return moviesdates.moviesdates_parser(search_movie_bool=False)
	if 'show_torrent_sources' in mode:# telemedia
			from indexers import dialogs
			return dialogs.show_torrent_sources(params)
	if 'trakt.' in mode:
		if '.list' in mode:
	...
	if mode == 'remove_downloads':# telemedia
		import shutil,os,xbmcvfs,xbmcgui,xbmcaddon
		telemedia=xbmcaddon.Addon('plugin.video.telemedia')
		directory=telemedia.getSetting('directory_mod')
		choice = xbmcgui.Dialog().yesno('Cobra', "פעולה זה תמחק את כל התוכן שהורדתם, האם להמשיך?", yeslabel="[B]כן[/B]", nolabel="[B]לא[/B]")

		if choice == 1:
			if directory=='true':
				path=telemedia.getSetting('movie_download_directory')
				try:shutil.rmtree(os.path.join(path,'documents'))
				except:pass
				try:shutil.rmtree(os.path.join(path,'videos'))
				except:pass
				try:shutil.rmtree(os.path.join(path,'temp'))
				except:pass
			try:shutil.rmtree(os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.cobra","Telemedia Downloads"))
			except:pass
			try:shutil.rmtree(os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.cobra","Movies Downloads"))
			except:pass
			try:shutil.rmtree(os.path.join(xbmcvfs.translatePath("special://userdata/"),"addon_data","plugin.video.cobra","TV Show Downloads"))
			except:pass
			notification('נמחק')
		else:
			notification('בוטל')
	...
	if mode == 'open_settings':
		...
		return open_settings(_get('query', '0.0'), _get('addon', 'plugin.video.cobra'))
