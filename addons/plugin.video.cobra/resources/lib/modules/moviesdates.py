...
def MoviesOnlineDates_results_reader(search_movie_bool, title=None, original_title=None):
    ...
    if not messages and search_movie_bool=='next':   
        iteration_order = reversed(json_data)
        reversed(json_data)
        for message in iteration_order:
...
def moviesdates_parser(search_movie_bool, title=None, original_title=None):
    
    window_header = f"[COLOR yellow]{ls(400017)} ({title})[/COLOR]" if search_movie_bool else f"[COLOR FF5CFF34]{ls(400018)}[/COLOR]"
