...
class CheckSettings:
	def run(self):
		...
		addon_object.setSetting('extras.movie.button12', 'show_trakt_manager')# מתקן את השגיאה בתקציר הסרטים - לבטל אחרי תקופה מסויימת 29:10:2023
		addon_object.setSetting('extras.tvshow.button12', 'show_trakt_manager')
...
class DatabaseMaintenance:
	def run(self):
		...
		due_clean = int(get_setting('cobra.database.maintenance.due', '0'))
		if due_clean == 0:
...
class FirstRunActions:
	def run(self):
		...
		if get_setting('first_use', 'false') == 'true':
			lines = ('By default, Cobra has [B]Local Folders[/B], [B]Debrid Cloud[/B], [B]Furk[/B] and [B]Easynews[/B] scrapers.',
			'These scrapers are designed for use on media you have purchased and organized through these services.',
			'Cobra is [B]unable[/B] to search online for sources outside of these services.',
			'If you wish to use an external scraper package within Cobra, you must select a module at[CR][B]Cobra Settings->Providers->Choose External Scrapers Module.[/B]',
			'The use of external scrapers is outside of the intended use of Cobra, and will not be supported by Cobra.')
...
class ReuseLanguageInvokerCheck:
	def run(self):
		...
		current_addon_setting = get_setting('cobra.reuse_language_invoker', None)
		...
		if invoker_instance.data != current_addon_setting:
			...
			if not get_setting('cobra.auto_invoker_fix') == 'true' and not confirm_dialog(text='%s\n%s' % (ls(33021), ls(33020))):
	...
	def run(self):
		...
		while not abort_requested():
			...
			while not any([context_visible, info_visible]) and not abort_requested():
				...
				if not external() or get_infolabel(listitem_property_str % 'cobra.external') == 'true':
					run_custom = True
					custom_context_params = get_infolabel(listitem_property_str % 'cobra.options_params')
					custom_main_context_params = get_infolabel(listitem_property_str % 'cobra.context_main_menu_params')
					custom_info_params = get_infolabel(listitem_property_str % 'cobra.extras_params')
...
class PremiumExpiryCheck:
	def run(self):
		...
		try:
			self.run_check()
			if self.make_message(): ok_dialog('Cobra - Premium Accounts Expiring...', '\n'.join(self.message))
			self.expired_premium = [i for i in self.days_remaining if i[1] == 0]
			if self.expired_premium and confirm_dialog(heading='Cobra - Premium Accounts Expired', text='Disable Expired Accounts?', ok_label=32839, cancel_label=32840): self.revoke()
		except: pass
	...
	def run(self, sender, method, data):
		if sender == 'xbmc':
			if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
				...
				logger('OnNotificationActions', 'PAUSING Cobra Services Due to Device Sleep')
			elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
				clear_property(pause_services_prop)
				logger('OnNotificationActions', 'UNPAUSING Cobra Services Due to Device Awake')
