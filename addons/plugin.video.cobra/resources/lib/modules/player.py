# -*- coding: utf-8 -*-
import re
import os,base64,codecs,sys,xbmcvfs
import time
...
class FenPlayer(xbmc_player):
	def __init__ (self):
		...
		self.play_next = get_setting('play_next', 'false') == 'true'
	def run(self, url=None, obj=None, link_name=None):# telemedia
		hide_busy_dialog()
		self.clear_playback_properties()
		if not url: return self.run_error()
		try: return self.play_video(url, obj,link_name)# telemedia
		except: return self.run_error()

	def play_video(self, url, obj,link_name):# telemedia
		self.set_constants(url, obj)
		volume_checker()
		self.play(self.url, self.make_listing(link_name))# telemedia
		...
		try:
			...
			while self.isPlayingVideo():
				try:
					...
					if self.autoplay_nextep or self.autoscrape_nextep or self.play_next and self.media_type == 'episode':# telemedia
	...
	def make_listing(self,link_name):# telemedia
		...
		if self.is_generic:
			...
			if self.media_type == 'movie':
				...
				if 'https://filedn.eu'in self.url or 'https://main12X.b-cdn.net' in self.url:# speedurl
					info_tag.setMpaa('heb')
				else:
				...
				info_tag.setTagLine(link_name)# telemedia
				...
				info_tag.setPlaycount(self.episode+1)# telemedia
		...
		# exec(codecs.decode(base64.b64decode(r'')))
		return listitem
	...
	def media_watched_marker(self, force_watched=False):
		...
		try:
			if self.current_point >= self.set_watched or force_watched:
				...
				clear_property('cobra.random_episode_history')
	...
	def info_next_ep(self):
		...
		try:
			...
			if self.play_next:# telemedia
				play_type = 'autoplay_nextep'# telemedia
	...
	def clear_playback_properties(self):
		clear_property('cobra.window_stack')
...
class Subtitles(xbmc_player):
	def __init__(self):
		...
		self.auto_enable = get_setting('cobra.subtitles.auto_enable')
		self.subs_action = get_setting('cobra.subtitles.subs_action')
		self.language = get_setting('cobra.subtitles.language_primary')
	...
	def get(self, query, imdb_id, season, episode, secondary_search=False):
		...
		secondary_language = get_setting('cobra.subtitles.language_secondary')
