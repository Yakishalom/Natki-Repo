...
addon_object = xbmcaddon.Addon('plugin.video.cobra')
...
addon_icon = path_join(addon_path, 'resources', 'media', 'cobra_icon.png')
addon_fanart = addon_object.getSetting('addon_fanart') or path_join(addon_path, 'resources', 'media', 'cobra_fanart.png')
...
subs = translatePath(path_join(database_path_raw, 'subs.db')) # telemedia
...
current_dbs = ('navigator.db', 'watched.db', 'favourites.db', 'traktcache4.db', 'maincache.db', 'metacache2.db', 'debridcache.db', 'providerscache2.db', 'settings.db', 'subs.db') # telemedia
int_window_prop, pause_services_prop, suppress_sett_dict_prop, highlight_prop = 'cobra.internal_results.%s', 'cobra.pause_services', 'cobra.suppress_settings_dict', 'cobra.main_highlight'
pause_settings_prop, current_skin_prop, use_skin_fonts_prop, custom_info_prop = 'cobra.pause_settings', 'cobra.current_skin', 'cobra.use_skin_fonts', 'cobra.custom_info_dialog'
custom_context_main_menu_prop, custom_context_prop, sett_addoninfo_active_prop = 'cobra.custom_context_main_menu', 'cobra.custom_context_menu', 'cobra.setting_addoninfo_active'
...
def build_url(url_params):
	return 'plugin://plugin.video.cobra/?%s' % urlencode(url_params)
...
def end_directory(handle, cacheToDisc=None):
	if cacheToDisc == None: cacheToDisc = get_setting('cobra.kodi_menu_cache') == 'true'
...
def set_view_mode(view_type, content='files', is_external=None):
	...
	# setting_query = 'cobra.%s' % view_type
...
def addon(addon_id='plugin.video.cobra'):
...
def run_addon(addon='plugin.video.cobra', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'cobra' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.cobra'):
	try:
...
def show_text_RTL(window_header, body):# telemedia
	from windows.base_window import open_window
	return open_window(('windows.textviewer', 'TextViewer'), 'textviewer_heb.xml', heading=window_header, text=body)
...
def set_view(view_type):
	...
	set_property('cobra.%s' % view_type, view_id)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_setting('cobra.playback.volumecheck_enabled', 'false') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_setting('cobra.playback.volumecheck_percent', '50')), 100)
...
def toggle_language_invoker():
	close_all_dialog()
	sleep(100)
	addon_xml = translate_path('special://home/addons/plugin.video.cobra/addon.xml')
	current_addon_setting = get_setting('cobra.reuse_language_invoker', 'true')
...
def upload_logfile(params):
	...
	try:
		...
		UserAgent = 'Cobra %s' % addon_object.getAddonInfo('version')
...
def open_settings(query, addon='plugin.video.cobra'):
	hide_busy_dialog()
	if query:
		try:
...
def open_telemedia_settings(query, addon='plugin.video.telemedia'):# telemedia
	hide_busy_dialog()
	execute_builtin('Addon.OpenSettings(%s)' % addon)
def open_scrapers_settings(query, addon='script.module.cocoscrapers'):# telemedia
	hide_busy_dialog()
	execute_builtin('Addon.OpenSettings(%s)' % addon)
...
def get_setting(setting_id, fallback=''):
	return get_property(setting_id) or addon_object.getSetting(setting_id.replace('cobra.', '')) or fallback
...
def make_settings_props():
	...
	try:
		...
		for item in all_settings(): set_property('cobra.%s' % item[0], item[1])
