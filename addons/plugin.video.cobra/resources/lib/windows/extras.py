...
setting_base, label_base, ratings_icon_base = 'cobra.extras.%s.button', 'button%s.label', 'fen_flags/ratings/%s'
...
class Extras(BaseDialog):
	...
	def show_trailers(self):
		if not self.youtube_installed_check():
			import xbmc
			return xbmc.executebuiltin("RunPlugin(plugin://plugin.video.youtube)" )#self.notification('Youtube Plugin needed for playback')
