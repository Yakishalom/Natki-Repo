...
# telemedia
CLIENT_ID, CLIENT_SECRET = 'e3a8d1c673dfecb7f669b23ecbf77c75fcfd24d3e8c3dbc7f79ed995262fa1db', '73bee6aeee29cb75db4d8771458a440017f7cfe842e85f457ed9d81f7910b349'# telemedia
...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	def send_query():
		...
		if with_auth:
			try:
				try: expires_at = float(get_setting('cobra.trakt.expires'))
				except: expires_at = 0.0
				if time.time() > expires_at: trakt_refresh_token()
			except: pass
			token = get_setting('cobra.trakt.token')
...
def trakt_refresh_token():
	data = {        
		...
		'grant_type': 'refresh_token', 'refresh_token': get_setting('cobra.trakt.refresh')}
...
def trakt_revoke_authentication(dummy=''):
	data = {'token': get_setting('cobra.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('cobra.trakt.calendar_previous_days', '3'))
		future_days = int(get_setting('cobra.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	if not get_setting('cobra.trakt.user', '') and not force_update: return 'no account'
