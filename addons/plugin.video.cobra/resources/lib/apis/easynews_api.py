...
def import_easynews():
	...
	# if get_setting('cobra.easynews.api_version') == '0': return EasyNewsAPI()
...
class EasyNewsAPI:
	def __init__(self):
		...
		self.username = get_setting('cobra.easynews_user')
		self.password = get_setting('cobra.easynews_password')
		...
		def _process():
			for item in files:
				try:
					...
					from modules.kodi_utils import logger
					logger('COBRA easynews API Exception', str(e))
	...
	def get_farm_and_port(self, files):
		...
		if get_setting('cobra.easynews.use_custom_farm', 'False') == 'True': dl_farm, dl_port = get_setting('cobra.easynews.farm', dl_farm), get_setting('cobra.easynews.port', dl_port)
