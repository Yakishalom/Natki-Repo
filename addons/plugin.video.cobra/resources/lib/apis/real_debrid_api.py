...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting('cobra.rd.client_id')
		if self.client_ID == '': self.client_ID = 'X245A4XAIBGVM'
		self.token = get_setting('cobra.rd.token')
		self.secret = get_setting('cobra.rd.secret')
		self.refresh = get_setting('cobra.rd.refresh')
		self.device_code = ''
